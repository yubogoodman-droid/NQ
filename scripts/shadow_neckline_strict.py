...
相對原腳本新增：
- 收盤價跌破頸線 + SMA14（不用影線刺破）
- 收盤跌破深度 >= 0.8%
- 當根收陰
- 前一根已收在頸線下（確認）
- 收盤 < SMA25 且 SMA14 < SMA25
- 肩距 12~60 根、兩肩對稱 < 10%
- 24h 漲幅上限 60%
- 同幣冷卻 150 分鐘
"""
...
SCAN_INTERVAL = 60
REPORT_GAP_MINUTES = 150
MAX_CHG24 = 60.0  # percent
MIN_CLOSE_BREAK_PCT = 0.8  # percent
# ===========================================
...
def detect_shadow_neckline_strict(df: pd.DataFrame, chg24: float | None):
    if len(df) < 250:
        return False, None
...
def main():
    ...
    print(
        f"📡 Strict 影線頸線監控中... (冷卻 {REPORT_GAP_MINUTES}m / "
        f"收盤確認+SMA25 / 24h<{MAX_CHG24}%)"
    )
    last_report_time = {}
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    has_signal, d = detect_shadow_neckline_strict(
