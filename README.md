...
1 分鐘圖：高檔做出**左高右平/略低**的雙頂（右峰不得再創高），頸線當時仍在 MA60 上方。第二頭確認後，**收盤同時跌破頸線與 MA60**（至少 8 點，避免均線追上吻線）才做空。  
**均線糾結不進**：MA5/10/20/30/60 帶寬（max−min）未滿 28 點就繼續等打開，不在 11:00 那種黏帶裡進場。  
1m 停損在左峰頭頂 +8；5m **+36**（避開 08-24 07:35 那種軋空掃損）。5m **收盤夾在 MA120 與 MA200 中間不空**（慢均還在托，只破 MA60 當假跌破）。目標 2R。鎖利下一根才生效：1m 見過 **1.6R 鎖 1.2R**；5m **0.8R 鎖 0.5R、1.2R 鎖 0.9R、1.6R 鎖 1.2R**。持倉最多 120 根，不重疊。風險需 50–220 點。

同一套規則另跑 **五分 K 對照**（轉折 15 分、雙頂間隔 20 分–4 小時）。1m 圖上粉紅虛線是已收盤的 5m MA60。
...
Yahoo 1m（2026-07-30 → 08-28）：**2 筆、勝率 100%、+372 點**。  
五分 K 同一套規則對照：**7 筆、勝率 85.7%、+749 點**（不用時間過濾；3 筆收盤夾在 MA120/200 中間被擋）。  
08-21 00:35 假跌破已濾。#2 鎖利 **+63**，08-24 07:00 **+156**，08-25 19:35 **+99**。今日 1m #2：**11:40** / 29678 / 逾時 **+205.5**。

## NQ 一分 K 低檔 W 底站上 MA60 做多（M 頭鏡像）

同一套規則整組反向：低檔做出**左低右平/略高**的雙底（右底不得再破底），頸線當時仍在 MA60 下方。第二底確認後，**收盤同時站上頸線與 MA60**（至少 8 點）才做多。均線糾結、慢均夾心、停損緩衝、2R、鎖利檔位都跟空單同一套。

```bash
python3 examples/nq_w_bottom_ma.py --period 30d --pages
python3 examples/test_nq_w_bottom_ma.py
```

報告：`docs/nq-w-bottom-ma/`（GitHub Pages）／`view.html`（內嵌圖）。  
這不是舊版「突破頸線就做多」的 W 底。

Yahoo 1m（2026-07-30 → 08-28）：**2 筆、勝率 100%、+171 點**（都在 08-24 那天反彈）。  
五分 K：**3 筆、勝率 66.7%、+32 點**（08-11 06:30 停損 −128 吃掉大半）。  
同一個月空單是 1m +372 / 5m +749，鏡像做多有邊但弱很多。
