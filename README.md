...
## 台股一分 K 掃描（多頭排列 × 站穩 MA240 兩根）

條件：

1. 成交金額前 200 名（證交所／櫃買已公布的交易日；盤中用前一交易日，不掃永豐全市場快照）
2. **濾掉 ETF**（代號 00/02 開頭或含英文字）、**金融股**（金控／銀行／保險／證券）與**股價 650 元以上**
3. 一分 K 的 **MA5 > MA10 > MA20**（多頭排列），MA5−MA20 ≥ 0.4%，MA20 與 MA240 差距 **0.4%～1.0%**
4. **從 MA240 下面穿上**，金叉前至少三根收在 240 下；**連續兩根收盤站穩**才通知，站穩時間 **09:10 以後**
5. 含該根的五分 K 收盤也必須高於五分 MA240

`MA240` 是一分 K 的 240 期均線（約 240 分鐘），不是日線 240 日。隔夜跳空與開盤 09:00–09:09 不算站穩。
...
1. 登入永豐，等商品合約下載穩定
2. 抓證交所／櫃買最近已公布的成交額前 200 名，按日補近幾天一分 K 當種子
