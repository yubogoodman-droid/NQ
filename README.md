...
那斯達克期貨（NQ）五分鐘 K 線的 **破底 W 底** 做多進場策略。

三個低點：**L1 左、L2 中間破底、L3 右**。
...
|------|------|
| L1 | 左腳波段低點 |
| L2 破底 | L1 與 L3 **之間的最低點**，必須低於 L1 至少 **0.10% 或 30 點**，之後收復 L1 |
| L3 | 右腳，與 L1 價差 ≤ 0.1%，且高於 L2 |
| 時間 | L1、L2、L3 須在 **2 小時內**完成（五分 K 最多 24 根） |
| 頸線 | L1～L3 之間最高點（量度目標用，不當作進場條件） |
| 進場 | L3 那根五分 K **收盤**（右腳經轉折確認） |
| 均線 | MA5/10/20/60/120/200 高低差須 ≥ **40 點**（糾結盤整不進） |
| 不追高 | 進場不得高於 MA200 超過 **120 點**；不得低於 MA60 超過 **80 點**；收盤不得低於 L1 超過 **10 點**；距頸線至少 **20 點**；L2 後 **4 根內**收復 L1 |
| 停損 | L3 下方 **20 點** |
| 停利 | 頸線 + (頸線 − L2) |
...
| `min_spring_pct` | 0.001 | L2 低於 L1 的最少深度（0.10%） |
| `min_spring_points` | 30 | L2 低於 L1 的最少點數（與百分比取較嚴） |
| `min_bounce_pct` | 0.001 | 破底前最少反彈（0.10%） |
| `max_reclaim_bars` | 4 | L2 之後幾根內須收復 L1 |
| `min_bars_between_lows` | 8 | L1 到 L3 最少間隔 |
| `max_bars_between_lows` | 24 | L1 到 L3 最多間隔（約 2 小時） |
| `max_pattern_hours` | 2.0 | L1 到 L3 實際時間上限 |
| `stop_below_l3_points` | 20 | 停損放在 L3 下方幾點 |
| `min_ma_span_points` | 40 | 進場時均線高低差下限（糾結則跳過） |
| `max_dist_above_ma200` | 120 | 進場高於 MA200 超過此點數則跳過 |
| `max_dist_below_ma60` | 80 | 進場低於 MA60 超過此點數則跳過 |
| `max_close_below_l1` | 10 | L3 收盤低於 L1 超過此點數則跳過 |
| `min_room_to_neckline` | 20 | 進場距頸線少於此點數則跳過 |
