...
1m 停損在左峰頭頂 +8；5m **+36**（避開 08-24 07:35 那種軋空掃損）。5m **收盤夾在 MA120 與 MA200 中間不空**（慢均還在托，只破 MA60 當假跌破）。5m 收盤還在 **MA200 上方超過 150 點且 1h 已破才不空**（#5；#6 那種 1h 沒破加回來）；已破 MA200 但 **1h MA60 還在下面超過 200 點且 MA60 仍往上也不空**（#8 砸到 200、大週期還沒測到）。5m 持倉後多一條 STRICT **破底翻平空**（進場不變）：進場瀑布不算破底；先做出場後擺動低再破那個低，15 根內收盤收回 MA20+MA30、MA5>MA10>MA20，且 MA20 已往上；貼下彎 MA20 不平。目標 2R。鎖利下一根才生效：1m 見過 **1.6R 鎖 1.2R**；5m **0.8R 鎖 0.5R、1.2R 鎖 0.9R、1.6R 鎖 1.2R**。持倉最多 120 根，不重疊。風險需 50–220 點。

同一套規則另跑 **五分 K** 與 **一小時 K** 對照（1h 0 筆：M 頭 18 → 高檔 6 → 未破 MA60 6）。1m 圖上粉紅虛線是已收盤的 5m MA60；5m 圖上紫虛線是已收盤的 1h MA60。

```bash
python3 examples/nq_m_head.py --period 60d --pages
...
Yahoo 1m 只能回看約 30 天（2026-07-30 → 08-28）：**2 筆、勝率 100%、+372 點**。  
五分 K 兩個月（2026-06-30 → 08-28）：**21 筆、勝率 81.0%、+1521 點**（進場同舊 21 筆 +1727；多 STRICT 破底翻平空）。07-13 22:10 由 −153 改 00:20 收盤平 **−3.2**。08-21 / 08-24 / 08-25 / 08-28 瀑布沒被砍。一小時 K 同一套規則 0 筆。今日 1m #2：**11:40** / 29678 / 逾時 **+205.5**。
