...
停損在盤整低點下方 5 點；目標 2R。走到 **1R 後停損移到 +0.3R**（移動停利），後面拉回就鎖一截，2R 還在。連續 2 根 1 分收回到盤整高點下，當成突破失敗、用收盤離場。不追 1 分實體已經超過 40 點的長 K。進場當下 **5 分收盤要在 5 分 MA20 上方，也要站上日線 30 日均線**。
...
近 10 天 Yahoo 1m（2026-08-14 → 08-24）：**16 筆、勝率 37.5%、+209.8 點**（改前必須站上 5 分 MA200：7 筆、57.1%、+192.4）。
近一個月 Yahoo 1m（2026-07-26 → 08-24）：**32 筆、勝率 34.4%、+223.1 點**（改前 21 筆、42.9%、+196.9）。這段時間大多站在 30 日均線上，過濾比 5 分 MA200 鬆。樣本仍少，只是回測不是保證。
預覽目前是這份一個月的圖。
你那張圖 **08-24 07:32 @ 29217.75** 當時 30 日均線約 29183，**會觸發**（移動停利 +15.3）。
