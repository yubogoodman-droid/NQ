...
訊號只在 **1 分鐘** 上抓：下跌後短均線（MA5–MA60）糾結、窄幅盤整（長下影會修剪）。進場是 **第一次收盤站上 MA200**，且 **MA5>MA10>MA20>MA30**，**MA60、MA120 都在 MA200 下方**。不要等後面那根放量長綠。

停損在盤整低點下方 5 點；目標 2R。走到 **1R 後停損移到 +0.3R**（移動停利），後面拉回就鎖一截，2R 還在。連續 2 根 1 分收回到盤整高點下，當成突破失敗、用收盤離場。不追 1 分實體已經超過 40 點的長 K。進場當下 **5 分收盤要在 5 分 MA20 與 5 分 MA30 上方**。
...
**11:29 為什麼舊規則沒抓到：** 11:21 那根急殺針把 10–18 根箱子拉到 58–64 點、8 條均線帶約 73 點，超過舊上限（區間 36、帶寬含 MA100/120/200）。現在改成 **7–18 根、區間 50 點、只用 MA5–MA60 量帶寬、下影超過 15 點不計進箱底**，所以 11:29 會進。
...
放寬箱子後 **08-24 07:32 @ 29217.75** 仍會觸發；**08-24 11:29 @ 29121.25**（11:21 探 29038.75 後收回 MA200）現在也會抓到。多出來的筆（例如 08-20 09:36 −120）把 30 天總分打成負。樣本很小，回測不是保證。
