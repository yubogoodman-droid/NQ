...
| 條件 | 說明 |
|------|------|
| L1 | 波段低點（左右各 3 根 K 確認） |
| 反彈 | L1 之後先彈，高點距離 L1 至少 0.10% |
| 破底 | 反彈後 low 低於 L1 至少 0.06% 或 10 點 |
| 收復 | 破底後 12 根內收盤站回 L1 |
| L2 | 收復後右底，與 L1 價差 ≤ 0.1%，且高於破底 |
| 進場 | 收盤突破頸線（L1～L2 之間最高點） |
| 停損 | L2 |
| 停利 | 量度漲幅：目標 = 頸線 + (頸線 − 破底低點) |
...
| `low_tolerance_pct` | 0.001 | L1/L2 價差容忍（0.1%） |
| `min_spring_pct` | 0.0006 | 破底最少深度（0.06%） |
| `min_bounce_pct` | 0.001 | 破底前最少反彈（0.10%） |
| `max_reclaim_bars` | 12 | 破底後幾根內須收復 |
| `min_bars_between_lows` | 8 | L1 到 L2 最少間隔 |
| `max_bars_between_lows` | 80 | L1 到 L2 最多間隔 |
