...
兩腳 W：左腳 L1，右腳 L2 跌破 L1（掃停），收復後突破頸線。
...
| 條件 | 說明 |
|------|------|
| L1 | 左腳波段低點（左右各 3 根 K 確認） |
| 頸線 | L1 與 L2 之間的最高點 |
| L2 破底 | 右腳低於 L1 至少 0.04% 或 8 點，且不超過 0.40% |
| 收復 | L2 之後 12 根內收盤站回 L1 |
| 進場 | 收盤突破頸線 |
| 停損 | L2（破底） |
| 停利 | 頸線 + (頸線 − L2) |
...
| `min_spring_pct` | 0.0004 | L2 低於 L1 的最少深度（0.04%） |
| `max_spring_pct` | 0.004 | L2 低於 L1 的上限（0.40%，避免崩跌） |
| `min_bounce_pct` | 0.001 | 頸線相對 L1 最少高度（0.10%） |
| `max_reclaim_bars` | 12 | L2 之後幾根內須收復 L1 |
| `min_bars_between_lows` | 5 | L1 到 L2 最少間隔 |
| `max_bars_between_lows` | 60 | L1 到 L2 最多間隔 |
