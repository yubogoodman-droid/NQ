...
<p class="muted">上面日盤 09:30–15:45 ET；下面附全時段（含夜盤）給研究。破底 → 收復粉紅 MA20（{escape(ma20_note)}）→ 離開後回踩進場。停損在破底下方，目標 1.5R；持有滿 {ma_exit_after} 根若收破 MA20 出場。進場若貼著下彎的 5m MA60（40 點內）則略過。{" 每筆附進場當下五分 K 對照。" if interval == "1m" else ""}</p>
