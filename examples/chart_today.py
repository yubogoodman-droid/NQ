...
def main() -> None:
    parser = argparse.ArgumentParser(description="產生 NQ 破三小時低 5/20站上MA60 HTML")
    ...
    if args.report or args.pages:
        ...
        if days:
            title = f"NQ 破三小時低 5/20站上MA60 — 近一個月" if days >= 28 else f"NQ 破三小時低 5/20站上MA60 — 近 {days} 天"
        else:
        ...
        title = f"NQ 五分K 破三小時低 5/20站上MA60 — {today} ({args.symbol})"
