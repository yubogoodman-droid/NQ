...
現行規則（無五分 MA60 斜率）：
  ...
  9. 停損後 30 分鐘內，五分 K 站回 MA5/10/20/60 且 MA5>10>20 多排，再進一次；停損設定同上。只再進一次。
...
def stands_on_5m_mas(close: float, ma5: float, ma10: float, ma20: float, ma60: float) -> bool:
    """五分 K 收盤站上 MA5/10/20/60，且 MA5>10>20 多排（60 不進排列）。"""
    vals = (close, ma5, ma10, ma20, ma60)
    if any(not np.isfinite(v) for v in vals):
        return False
    return close > ma5 and close > ma10 and close > ma20 and close > ma60 and ma5 > ma10 > ma20
...
def make_reentry_signal(
    ...
    *,
    ...
    """停損後 window_minutes 內，五分 K 站回 MA5/10/20/60 且 5>10>20 多排則再進一次。"""
    close = df["Close"].to_numpy(float)
    n = len(close)
...
<p class="muted">MA5&gt;10&gt;20&gt;30&gt;60 · 站上MA200連3且距≤30 · 破2h低後1小時 · 先前連15根在MA200下 · 9:30–10:00不進 · 紅K長上影跳過 · SL=MA200−10 / TP=+100 · 浮盈+60改保本 · 停損後30分內五分K站回MA5/10/20/60且5&gt;10&gt;20多排再進一次 · 5m / 15m / 1h 圖只對照</p>
<div class="cards">
