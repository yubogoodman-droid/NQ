#!/usr/bin/env python3
"""台股十五分 K 回測：MA5/10/20 多頭發散，當根收盤站上十五分 MA200，小時K在 MA5／10／20 之上。"""
...
from tw.backtest_5m import BacktestConfig, run_15m_backtest
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="台股十五分K多頭發散＋收盤站上十五分MA200＋小時K在MA5/10/20上，回測近 N 個交易日"
    )
...
def main() -> int:
    ...
    print()
    print(f"十五分K回測 {result.days[0].isoformat()}～{result.days[-1].isoformat()}  共 {len(result.hits)} 則通知")
