...
SEEN_PATH = ROOT / "output" / "binance_5m_short_seen.json"
...
def break_ma200(d: dict, i: int) -> bool:
    """前一根收在 MA200 上，這一根收盤跌破。"""
    if i < 1 or i >= len(d["c"]):
        return False
...
def pick_chart_hits(hits: list[dict], limit: int, extra: list[dict] | None = None) -> list[dict]:
    limit = max(0, int(limit))
    chosen: list[dict] = []
    seen: set[tuple] = set()

    def add(h: dict) -> bool:
        if limit <= 0 or h.get("60m") is None:
            return False
        key = (h["symbol"], h["t"])
    ...
    for h in scored[:half]:
        add(h)
    for h in reversed(scored):
        if len(chosen) >= limit:
            break
        add(h)
    if len(chosen) < limit:
...
def write_report(
    ...
    *,
    ...
    chart_hits = pick_chart_hits(hits, chart_limit, extra=pin_hits)
    cards = []
...
<title>幣安 5m 空頭排列 · {escape(period)}</title>
<style>
...
.tag{{font-size:11px;padding:3px 8px;border-radius:999px;border:1px solid #30363d;color:#ffa198}}
...
def test_telegram() -> int:
    apply_keys()
    ok = telegram_send("5m 空頭排列監看測試\n如果你看到這則，Telegram 已通。")
    print("Telegram 測試", "成功" if ok else "失敗（檢查 token / chat id）")
    return 0 if ok else 1


def main() -> int:
    p = argparse.ArgumentParser(description="幣安 5m 空頭排列 Telegram 監看")
    ...
    p.add_argument("--days", type=int, default=2, help="回測天數（預設 2）")
    p.add_argument("--pages", action="store_true", help="寫入 docs/binance-5m-short/")
