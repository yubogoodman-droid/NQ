...
<body>
  <div class="page">
    ...
    <div class="summary">
      ...
      排行時間 {rank_time}<br/>
      前 100 名 → 濾掉股價 {result.price_dropped}、ETF {result.etf_dropped} → 掃描 {len(result.candidates)} 檔 → 均線糾結 {result.tangled_dropped} → 五分未明顯站上MA200 {result.below_5m_dropped}
    </div>
    {hit_rows}
...
def _write_markdown(result: ScanResult, path: Path, *, chart_rel: Path) -> None:
    ...
    lines = [
        ...
        f"- 前 100 → 濾掉股價 {result.price_dropped}、ETF {result.etf_dropped} → 掃描 {len(result.candidates)} → 均線糾結 {result.tangled_dropped} → 五分未明顯站上MA200 {result.below_5m_dropped}",
        "",
    ]
