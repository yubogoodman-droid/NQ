...
class SignalTests(unittest.TestCase):
    ...
    def test_lookback_finds_earlier_cross_today(self) -> None:
        closes = [100.0] * 204 + [110.0] + [111.0, 112.0]
        df = _bars(closes)
    ...
    def test_until_keeps_friday_and_ignores_monday(self) -> None:
        fri = list(pd.date_range("2026-08-14 09:00", periods=206, freq="1min", tz="Asia/Taipei"))
        mon = list(pd.date_range("2026-08-17 09:00", periods=3, freq="1min", tz="Asia/Taipei"))
        closes = [100.0] * 204 + [110.0, 111.0] + [112.0, 113.0, 114.0]
        df = pd.DataFrame(
            {
                "open": closes,
                "high": [c + 0.5 for c in closes],
    ...
    def test_cross_after_open_is_entry(self) -> None:
        ...
        closes = [100.0] * 200 + [100.0] * 9 + [110.0, 111.0]
        df = pd.DataFrame(
            {
                "open": closes,
                "high": [c + 0.5 for c in closes],
    ...
    def test_weave_along_ma200_is_not_breakout(self) -> None:
        closes = [100.0] * 198 + [110.0, 99.0, 110.0, 111.0]
        self.assertIsNone(latest_ma200_breakout_bullish(_bars(closes)))

    def test_first_cross_is_entry_not_later_recross(self) -> None:
        closes = [100.0] * 204 + [110.0, 111.0, 90.0, 90.0, 90.0, 120.0, 121.0]
        df = _bars(closes)
        snap = latest_ma200_breakout_bullish(df)
        self.assertIsNotNone(snap)
        assert snap is not None
    ...
    def test_keeps_open_ma_stack_like_jinju(self) -> None:
        ...
        self.assertGreater(snap.ma20_ma200_gap_pct, 0.004)
        self.assertLess(snap.ma20_ma200_gap_pct, 0.010)
    ...
    def test_drops_hug_when_ma20_glued_to_ma200(self) -> None:
        snap = AlertSnapshot(
        ...
        self.assertLess(snap.ma20_ma200_gap_pct, 0.004)
    ...
    def test_drops_huaxinke_when_ma20_far_from_ma200(self) -> None:
        ...
        self.assertGreater(snap.ma20_ma200_gap_pct, 0.010)
        self.assertFalse(ma20_near_ma200(snap))
