(function (root, factory) {
  ...
  function calcFee(price, qty, market, settings) {
    if (!settings.autoFee || market !== "TW") return 0;
  ...
  function buildBooks(trades) {
    ...
    for (const t of sortedTrades(trades)) {
      ...
      if (t.side === "buy") {
        ...
        book.sellQty += qty;
        let left = qty;
        const netPer = price - extra / qty;
        while (left > 0 && book.lots.length && book.lots[0].qty > 0) {
          const lot = book.lots[0];
          const match = Math.min(left, lot.qty);
          const pnl = (netPer - lot.costPerShare) * match;
          book.realized += pnl;
          realizedHere += pnl;
          lot.qty -= match;
          left -= match;
  ...
  function positionsFrom(books, quotes) {
    ...
    for (const book of books.values()) {
      ...
      for (const lot of book.lots) {
        qty += lot.qty;
        cost += lot.qty * lot.costPerShare;
      }
      const avgCost = qty ? cost / qty : 0;
      const q = quotes[`${book.market}:${book.symbol}`];
      const quote = q ? Number(q.price) : null;
      const marketValue = quote != null ? quote * qty : null;
      const unrealized = quote != null && qty ? (quote - avgCost) * qty : null;
      out.push({
        market: book.market,
        symbol: book.symbol,
        name: book.name,
  ...
  return { tradeKey, sortedTrades, calcFee, calcTax, buildBooks, positionsFrom };
});
